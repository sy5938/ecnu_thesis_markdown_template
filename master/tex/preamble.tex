...
% 官方参考 PDF 使用 SimHei；优先检测并使用 SimHei（其本身是单一粗黑体字重）。
% 当前 macOS 未安装 SimHei 时，回退到 STHeiti，并用 AutoFakeBold=2 保证
% \bfseries 下仍有足够的黑体粗度；若两者都不存在，最终回退到 Heiti SC Medium。
% 各分支显式设置 Scale=1，消除 ctex/fontspec 对黑体字体度量的额外缩放。
\IfFontExistsTF{SimHei}{
  \setCJKfamilyfont{zhhei}[Scale=1]{SimHei}
}{
  \IfFontExistsTF{STHeiti}{
    \setCJKfamilyfont{zhhei}[Scale=1,AutoFakeBold=2]{STHeiti}
  }{
    \IfFontExistsTF{Heiti SC}{\setCJKfamilyfont{zhhei}[Scale=1]{Heiti SC Medium}}{}
  }
}
...
\dottedcontents{section}[1.5cm]{\zihao{-4}\heiti}{1.6em}{4pt}
\dottedcontents{subsection}[2.1cm]{\zihao{-4}\heiti}{2.3em}{4pt}

% 图表目录样式：宋体，小四
\dottedcontents{figure}[1.5cm]{\zihao{-4}}{2.4em}{4pt}
\dottedcontents{table}[1.5cm]{\zihao{-4}}{2.4em}{4pt}
